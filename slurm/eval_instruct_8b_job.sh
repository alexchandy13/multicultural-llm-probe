#!/bin/bash
#SBATCH --job-name=eval_instruct_8b
...
#SBATCH --output=slurm/eval_instruct_8b.%j.out
#SBATCH --error=slurm/eval_instruct_8b.%j.err

# Full eval suite for Llama 3.1 8B Instruct (Meta's official instruct model)
# using the standard protocol: nfs + mpw + us-probe + batch calibration.
# Submit with: sbatch slurm/eval_instruct_8b_job.sh
...
MS=8b_instruct
echo "[eval_instruct_8b] model_size=$MS"
