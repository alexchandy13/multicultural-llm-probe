#!/bin/bash
#SBATCH --job-name=culture_sftdpo_alpaca
...
#SBATCH --output=slurm/sftdpo_alpaca.%j.out
#SBATCH --error=slurm/sftdpo_alpaca.%j.err

# C4a — DPO on top of an existing Alpaca SFT adapter (robustness variant for C4).
#
# Depends on the Alpaca SFT checkpoint at checkpoints/sft_alpaca/ being present
...
# init_adapter_path (which already points at checkpoints/sft_alpaca).
